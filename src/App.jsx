...
function App() {
  ...
  return (
    <div className="h-screen bg-canvas transition-colors duration-300 flex flex-col overflow-hidden">
      <header className="flex-shrink-0 px-4 md:px-6 py-3 md:py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-hairline bg-surface-elevated/20 z-10 shadow-sm">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`p-2 rounded-lg transition-colors border shadow-sm ${isSettingsOpen ? 'bg-primary text-black border-primary' : 'bg-surface-card hover:bg-surface-elevated text-body border-hairline'}`}
      ...
      <main className="flex-1 overflow-hidden flex flex-row relative">
        ...
        {isSettingsOpen && (
          <div className="absolute inset-y-0 left-0 z-30 md:relative md:z-0 w-full md:w-1/3 lg:w-1/4 md:min-w-[320px] md:max-w-[400px] h-full shadow-2xl md:shadow-none bg-canvas animate-in slide-in-from-left-10 duration-300">
            <SettingsSidebar />
          </div>
        )}
